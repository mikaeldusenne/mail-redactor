...
instance Show Mail_content where
  show (MC { getH = h, getContent = b}) = (unlines .  map unlines h) ++ show b
  show (Mail_Leaf l) = unlines l
  -- show (MC { getH = h, getContent = b}) = unlines ["§§§§§§§§§§§§§§§§",
  --                                                  "----Headers:----",
  --                                                  unlines . map unlines $  h,
  --                                                  "----Content:----",
  --                                                  unlines $ map (("§ "++) . show) b]

-- instance Read Mail_content where
--   readsPrec d r = 

isMultipartDeclaration = isInfixOf "content-type: multipart" . map toLower

findHeader h = safe_head . filter (isInfixOf h . map toLower . head)

-- extractMultipartBoundary s = filter (not . (=='"')) . drop 9 . dropWhile (not . isPrefixOf "boundary=")

-- readMultipartContent boundary content = f content
--   where f ('-':'-':xs) | isInfixOf boundary xs = 

-- read_mailcontent = f []
--   where f acc ("":ls) = Mail_content {getHeaders = MH acc,
--                                       getContent = read_mailcontent ls}
--         f acc (l:ls) | isMultipartDeclaration l = 
...
readMail l = MC {getH = h,
   ...
   where (h',b') = sliceOn [] l
         ...
         b = case multih of (Just mh) -> readMultipartContent (head mh) b'
                            Nothing -> [Mail_Leaf b']
                         
        -- readmail acc (l:xs)
        --   | isEmpty l                             = (acc, [Mail_Leaf xs])
        --   | readMultipartDeclaration l /= Nothing = (acc, readMultipartContent l xs)
        --   | otherwise                             = readmail (acc ++ [l]) xs
